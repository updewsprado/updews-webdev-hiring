...
class Controller extends Database {
	...
	public function setJsFile()
	{
		if(count($this->jsfile))
		{
			foreach($this->jsfile as $file){
				echo '<script src="'.BASE_HOST.'/'. BASE_FOLDER.'/'.$file.'"></script>'. PHP_EOL;
			}	
		}
	}
	...
	// get all the third party js 
	//@var $_file_ file
	public function getOutSideJsFile($_file_)
	{ 
		$this->outjsfile[] = $_file_;
	}
	// set all the third party js 
	//@var $_file_ file
	public function setOutSideJs()  
	{
		
		if(count($this->outjsfile))
		{
			foreach($this->outjsfile as $file){
				echo  '<script src="'.$file.'" type="text/javascript"></script>'.PHP_EOL;
			}
		}
	}
	...
	public function setCssFile() 
	{
		if(count($this->cssfile))
		{
			foreach($this->cssfile as $file){
				echo '<link href="'.BASE_HOST.'/'. BASE_FOLDER.'/'.$file.'" rel="stylesheet">'. PHP_EOL;
			}	
		}
	}
